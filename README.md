...
Нынещнюю версию можно развивать по нескольким направлениям

### 1. Улучшение качества поиска

| Направление | Описание |
|-------------|----------|
| **Смена модели эмбеддингов** | Заменить `multilingual-MiniLM` на `intfloat/multilingual-e5-large` или `BAAI/bge-m3`|
| **Адаптивный чанкинг** | Вместо фиксированного размера использовать Semantic Chunker или разбивку по смысловым абзацам|
| **Переранжирование (Reranking)** | Применить русскоязычный кросс-энкодер|
| **Гибридный поиск с BM25 (с улучшениями)** | Добавить лемматизацию (pymorphy2) и взвешенное ранжирование| 

### 2. Расширение функциональности

| Направление | Описание |
|-------------|----------|
| **Поддержка новых форматов** | DOCX, XLSX, PPTX, HTML, EML (письма). |
| **Мультимодальность** | Транскрипция аудио/видео (Whisper) для анализа совещаний, OCR для сканов. |
| **История с суммаризацией** | Вместо хранения полной истории использовать `ConversationSummaryMemory` для сжатия длинных диалогов. |

### 3. Оценка и контроль качества

| Направление | Описание |
|-------------|----------|
| **Расширение тестового набора** | Увеличить число вопросов (до 50–100) для более точной оценки. |
| **Faithfulness** | Автоматическая проверка, что ответ не противоречит контексту (с использованием второй LLM). |
| **A/B тестирование** | Сравнение разных версий системы (например, с/без reranking) на реальных запросах. |